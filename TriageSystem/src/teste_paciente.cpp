...
TEST_CASE("Teste exceções") {
    ...
    SUBCASE("Lançar exceção CpfInvalido") {
        const std::string cpf = "1234";
        CHECK_THROWS_AS(Paciente("Maria", 25, cpf, "Convênio B"), CpfInvalido&);
    }

    SUBCASE("Lançar exceção ConvenioInvalido") {
        const std::string convenio = "1234";
        CHECK_THROWS_AS(Paciente("João", 40, "12345678901", convenio), ConvenioInvalido&);
    }
}

TEST_CASE("Teste adicional") {
    Paciente paciente("Laura", 35, "98765432109", "Convênio F");
...
TEST_CASE("Teste adicional 2") {
    Paciente paciente("Pedro", 28, "56789012345", "Convênio G");
